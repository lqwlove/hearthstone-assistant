...
def build_deck_tools(db: Session, deck: Deck) -> list[Any]:
    ...
    def apply_deck_patch(ops: list[dict[str, Any]]) -> str:
        """改套：用 set_count 加卡/改数量/删卡（count=0 删除）。
        ops 示例: [{"op":"set_count","card_id":"126416","count":1}]
        card_id 须来自 wiki（wiki-query skill）/ archetype 参考表 / 用户给出；禁止编造。
        目标是组满合法 30 张；流程见 deck-edit。"""
        fresh = db.scalar(select(Deck).where(Deck.id == deck_id))
        if fresh is None:
            return "错误: 卡组不存在"
